...
test.describe("Question re-score (E2E)", () => {
  test("changing a score updates the entry without duplicating", async ({ browser }) => {
    ...
    await expect(host.getByTestId("question-scores-panel").first()).toBeVisible({ timeout: 10_000 });
    ...
    await expect(host.getByTestId("question-scores-panel").first()).toBeVisible();
