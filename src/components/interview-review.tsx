...
export function isReportUnavailableMarkdown(markdown: string | undefined): boolean {
  return !!markdown?.includes("Summary unavailable");
}
...
export function InterviewReviewPanel({
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-zinc-50 to-zinc-100 dark:from-zinc-950 dark:to-zinc-900 flex flex-col items-center p-4 py-10" data-testid="interview-review-panel">
      <Card className="w-full max-w-3xl shadow-lg border-zinc-200 dark:border-zinc-800">
        <CardHeader className="border-b border-zinc-100 dark:border-zinc-800">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <CardTitle className="text-2xl" data-testid="interview-review-title">Interview complete</CardTitle>
            ...
            <div className="space-y-3 mb-4">
              ...
              {sessionNotesGate && (
                <div className="rounded-lg border border-amber-200 dark:border-amber-900/60 bg-amber-50/80 dark:bg-amber-950/30 p-3 space-y-1.5">
                  ...
                  <textarea
                    data-testid="review-session-notes"
            ...
            <div className="space-y-4">
              {reportUnavailable ? (
                <div className="space-y-2">
                  <p className="text-sm text-amber-800 dark:text-amber-200">
                    The AI summary could not be generated. You can edit session notes below and try again.
                  </p>
                  {onRetryReport && role === "interviewer" && (
                    <Button
                      type="button"
                      variant="secondary"
                      size="sm"
              ...
              {sessionNotesGate && reportUnavailable && role === "interviewer" && (
                <div className="rounded-lg border border-amber-200 dark:border-amber-900/60 bg-amber-50/80 dark:bg-amber-950/30 p-3 space-y-1.5">
                  <label className="text-xs font-medium text-amber-950 dark:text-amber-100">
                    Session notes for the AI report
                    {sessionNotesGate.optional ? (
                      <span className="font-normal text-zinc-700 dark:text-zinc-300">
                        {" "}
                        — optional. Used as primary evidence alongside the live transcript.
                      </span>
                    ) : (
                      <span className="font-normal text-amber-900/90 dark:text-amber-200/90">
                        {" "}
                        — required (no live transcript was captured). At least {sessionNotesGate.minLength}{" "}
                        characters.
                      </span>
                    )}
                  </label>
              ...
              <article
                data-testid="interview-report-markdown"
