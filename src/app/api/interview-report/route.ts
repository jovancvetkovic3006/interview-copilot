...
export async function POST(req: NextRequest) {
  try {
    ...
    if (transcriptLines.length === 0) {
      spokenSection = "(no spoken transcript captured)";
...
3. Strengths observed — base these on the structured transcript summary AND chat. Cite evidence (short verbatim quotes when possible).
